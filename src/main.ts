...
async function main() {
  ...
  const getInstructions = async () => {
    ...
    if (!content) {
      return [persona, '---', `**Today is ${dateStr}.**`].join('\n')
    }
  ...
  toolCenter.register(createPaperAttributionTools(accountManager), 'paper-attribution')
  ...
  // ==================== Signal Router (fork: autonomous paper bot trigger) ====================
